...
//            filePath = new String("/mnt/sdcard/tencent/QQfile_recv/计算机 1.xls");
    ...
    private void initFiles() {
        //添加静态数据
        Files excel = new Files("2015上学期计算机科学与技术 开课计划书(静态数据）",R.drawable.ic_file_excel);
        fileList.add(excel);
        if (filePath != null){
            //提取打开Excel文件的路径，截取文件名部分，添加到文件列表中显示。
            String [] fileNames = filePath.split("/");
            Files excel2 = new Files(fileNames[fileNames.length-1],R.drawable.ic_file_excel);
            fileList.add(excel2);
            //获取Excel数据
...
//                Log.d(TAG,cursor.getString(cursor.getColumnIndex(columns[i])));
    ...
    private void initDatabase(Bundle data) {
        //这里是默认已经表结构的写法，也就是写死了的，只能导入跟计算机 1.xls文件结构一样的表，后期有时间再来重写，先写出个能make running。
        DbTools db = new DbTools(MainActivity.this,"ExcelData",null,1);
